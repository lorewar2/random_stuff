...
khm_df = pd.read_csv("souporcell3_1.tsv", sep="\t", header=None, usecols=[0, 1], names=["barcode", "cluster"])
...
print(filtered_df["donor_encoded"])

filtered_df["cluster_encoded"] = le.fit_transform(filtered_df["cluster"])

print(filtered_df["cluster"])
ari = adjusted_rand_score(filtered_df["cluster_encoded"], filtered_df["donor_encoded"])
